...
`ProjectContinuityDelta` is a bounded, project-owned refresh diagnostic derived from existing upsert mutations and presentation revision. `ProjectContinuityDirtyMarker` records the V3.9 continuity inputs—Agent candidate, Correction/revert and Fact-ingestion writes—on `ProjectHistorySnapshot`; it never scans or invokes a model. Every covered write advances a persistent project-local generation under the existing snapshot row lock. Refresh acknowledges only the dirty revision observed before collection, so repeated same-target writes and concurrent later writes cannot be collapsed and remain STALE. Database Agent candidates enter source collection as bounded PROCESS_EVIDENCE.
...
Current Project State is a model-free read projection of persisted corrected history. Its confirmed result is selected from visible Primary Stories in descending occurrence order; pinning and Supporting context cannot override newer Primary outcomes. The same corrected result flows into Gateway Snapshot/Brief, Context Package v2, Hermes, Frontend Overview and Obsidian indexes. GET consumers never scan or reinterpret semantics.
