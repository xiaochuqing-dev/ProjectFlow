...
状态：BLOCKED / NOT_RUN。真实 GLM 与 DeepSeek 已产生部分候选，但没有冻结并完成独立人工评分。

## 抽样规则

每个真实 Provider 至少冻结 30 个 Story 和 8 个 Chapter，覆盖 ProjectFlow、五类非代码项目、短历史、大历史、generic Commit、add/modify/remove/restore、merge/split、conflict、unknown reason、Supporting 和用户修正。固定 ID、Evidence 和 presentation revision 冻结后才允许评分。
...
Story 抽样数量：0。Chapter 抽样数量：0。人工平均分：NOT_RUN。独立第二评审者：无记录。由于真实 Provider 资格已经失败且人工门禁未运行，PR #15 必须保持 Draft。
