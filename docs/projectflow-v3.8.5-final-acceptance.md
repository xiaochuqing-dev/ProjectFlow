...
Current decision: V3.8.5 is `PASS_BY_EXPLICIT_OWNER_OVERRIDE`. PR #15 merged as `29c154eb618ca43edf58c631c14cc1d296e14f3f`; post-merge master required CI run `32652683003` passed every required job, and the root launcher passed from the same clean revision. The acceptance-backfill metadata is `acceptance-evidence/v3.8.5/final-acceptance-backfill.json`. The pre-signoff decision below is retained as historical evidence.
