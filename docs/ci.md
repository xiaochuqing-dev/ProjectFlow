...
V3.7.4 adds blocking deterministic gates for strong-fact status/promotion, explicit reason/deprecation/technical-debt evidence, Agent-result isolation, extensionless content discovery, 80,000-line Content Map/range/revision behavior, cross-chunk merging, project authorization, Agent candidates, Context Packages, prompt parity and Ground Truth leakage. Generated giant fixtures stay in the local Acceptance Archive; Git contains only the generator, specification, manifest, hashes and sanitized summaries.

Real GLM and DeepSeek runs remain explicit non-default acceptance work because they use private funded APIs. Both use the same Strong Fact contract and builder. Calibration and frozen Holdout use different classpath resources and output directories. Only normalized observations and aggregate diagnostics may be archived; keys, prompts, raw responses and reasoning are forbidden.
