...
状态：RC2 代码审计与本地确定性回归已完成；历史双 Provider 资格失败和 DeepSeek Dogfood 10/11 继续保留。RC2 新真实运行因 Secrets 缺失未执行，独立人工抽样和最终合并未完成，结论为 BLOCKED。
...
- `ProjectHistoryReconstructionTest`：包含最小模型合同和并发取消确定性回归，随 H2 全量 546 项 PASS。
...
- 历史 GLM `glm-5.2` 与 DeepSeek Chat 合同请求已在隔离进程完成，但 19-case qualification 均 FAIL；不能把合同 PASS 当成质量 PASS。RC2 新运行尚未发出请求。
