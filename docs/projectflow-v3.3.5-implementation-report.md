...
桌面启动复测已完成：`Desktop\\Start-ProjectFlow.bat` 指向当前仓库，底层启动器会从当前源码重新构建前端并通过 `mvn spring-boot:run` 启动后端；实际启动时前端 3000 和后端 8080 均就绪。此前仅是启动器文案仍显示 V3.3.4，已统一修正为 V3.3.5。
...
| 桌面启动器 | 通过 | `Start-ProjectFlow.bat` / `start-projectflow-embedded.ps1 -CheckOnly` 显示 V3.3.5；完整无浏览器启动完成前端构建、后端启动和健康检查。 |
