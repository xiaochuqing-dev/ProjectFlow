...
Before an existing embedded H2 database is opened, the launcher creates an H2 payload backup. The release migration strategy exports a bounded, trusted schema-state marker; `backup-projectflow.ps1` consumes that marker and refuses to create a usable manifest when the identity is missing or unknown. It writes a complete payload plus manifest and SHA-256. Restore first creates an emergency backup, validates the selected payload/hash and H2-openability in isolation, moves the current database into a retained recovery directory, re-opens the installed files before success, and rolls back on failure. It never deletes the last current or backup copy.
