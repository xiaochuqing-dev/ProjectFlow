...
V3.7.4 adds an Agent-oriented read layer without bypassing Gateway ownership:
...
| Versioned provenance package | `GET /api/projects/{projectId}/project-memory/context-package` |

Hermes maps these reads to read-only tools and `projectflow://projects/{projectId}/context` resources. The package is derived from persisted ProjectFlow state, not generated by a host model. A separate Agent-candidate API accepts validation work but cannot write `OBSERVED` or `VERIFIED`.
