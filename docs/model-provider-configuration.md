...
Reasoning-capable Responses models count hidden reasoning and visible JSON against one output budget. The task still starts from a bounded useful ceiling; the configured Provider ceiling must leave room for the single bounded truncation recovery. V3.7.4 GLM 5.2 acceptance uses a 65,536 Provider ceiling: Scout still starts at 16k, and only a detected truncation can use the explicitly configured Provider ceiling. DeepSeek V4 Pro remains at 32,000. No private reasoning parameter is sent unless the Provider explicitly declares support.
...
## V3.7.4 two-model acceptance profile

V3.7.4 validates one provider-neutral Strong Fact contract through two official/compatible protocol adapters. The main profile is GLM `glm-5.2` through Ark Coding OpenAI Responses. The compatibility profile is current DeepSeek `deepseek-v4-pro` through official OpenAI Chat Completions. Model names and protocol details may differ; Evidence allow-lists, fact statuses, promotion guards, schema normalization, bounded repair and cancellation rules do not.
