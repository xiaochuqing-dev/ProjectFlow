...
V3.8.5 RC3 adds claim-level Evidence attribution: every visible state is capped by Technical Atoms that directly match the same subject and action, while same-commit, same-Story and Supporting Evidence remain context only. Broad engineering-area subjects stop at OBSERVED; precise subjects keep Evidence-based implementation eligibility. Chapter synthesis must name a concrete supported phase result, independent Primary outcomes in a generic commit are separated deterministically, and Chapter repair uses its own v6 schema. Story v12 retains the validated deterministic title/summary when Provider wording omits an explicit supported result, and exposes that fallback through Story status and diagnostics. Round 1 and Round 2 are both `NEEDS_REVISION_NOT_APPROVED`; Round 3 is a new 30 Story/8 Chapter package with blank human fields. PR #15 remains Draft until the user explicitly reviews and approves Round 3.
