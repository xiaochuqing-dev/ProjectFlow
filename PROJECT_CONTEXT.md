...
V3.10 Release Readiness 已从最终 V3.9 master `dd5ee41b6afcbd7703fa0883dc115c11f4821447` 在独立分支启动，当前处于 architecture/acceptance freeze，尚未宣称实现或 PASS。审计确认版本化 schema migration、OS-backed Provider credential、local bind/auth 安全、稳定 data/log/config 目录、升级备份恢复、source-independent Windows runtime 和供应链/Windows gate 均为本阶段阻断项。高风险边界冻结在 `docs/projectflow-v3.10-release-readiness-architecture.md`，升级恢复与运行目录合同分别位于 `docs/projectflow-v3.10-upgrade-and-recovery-contract.md`、`docs/projectflow-v3.10-runtime-data-directory-contract.md`，36 个确定性用例位于 `docs/acceptance-evidence/v3.10/release-readiness-ground-truth.json`。实现不得放宽 unknown schema block、两阶段 credential migration、loopback default、data/install separation 或 no-plaintext/no-secret-artifact 规则；继续禁止 Tag、Release 和提前实现 V4 GUI。
