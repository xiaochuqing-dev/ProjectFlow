...
V4.0-A GUI Foundation + Information Architecture 已完成 Owner Review 小幅修订，状态为 READY_FOR_OWNER_REVIEW_UPDATED。Owner 已认可 Hybrid 项目中心、Current State 默认第一屏、Current State / History / Agent Context 核心层级、Evidence 按需下钻、旧入口渐进降级、semantic token + source-owned component + headless primitive 方向，以及 Desktop Shell defer；修订后的合同仍等待最终确认。全局只保留项目库、项目切换和全局设置；项目内保留当前状态、项目历程、Agent 上下文、项目设置与集成。Capability Map 是仅适合部分项目的可选次级视图，不是通用产品主轴。

V4.0-A 只冻结产品与技术 Foundation，不实现最终 GUI。ProjectFact 仍是唯一强事实来源；Current Project State 继续是 persisted、model-free、corrected-history derived read model。Current State 必须提供带文字的显式更新主动作并在刷新时保留旧可信结果。History 的 Chapter 时间阶段轴与 Evolution Thread 长期主题轴互补，Thread 可跨 Chapter。Agent Context 第一层只展示正式交接内容，Context Package 精确字段进入工程详情，最终用户导航标签由 V4-B 验证。Obsidian 配置归项目设置，真实可用的 Note/投影日常入口可出现在内容上下文；Provider CRUD 和凭据归全局设置，项目设置不复制管理系统。

设计系统推荐 ProjectFlow 自有语义 token、源码拥有的业务组件与成熟无障碍 headless primitive；V4-B 由 Project Library、Current State、Project History、Story + Evidence Drawer 等核心 prototype 驱动最小组件集合，不一次性实现完整 UI Kit。桌面壳结论仍为 DESKTOP_SHELL_DECISION = DEFERRED：优先用现有 Java + Next standalone loopback runtime 做 Electron/Tauri 可删除同机 PoC，WebView2 direct host 只作研究对照。V4-A Draft PR 不得自行 merge、Tag 或 Release；Owner 最终确认修订后才可进入 V4-B。
...
V3.9 Project Continuity Closure 已在生产与评测源码头 `f3c3adbd79206fc21a8a5209774a0b71ef47e185` 完成，并由 PR #17 合并为 master `d4bceed673d7cd630bc3b6663f673be6f2ac3c5b`。它继续复用来源 fingerprint/affectedFrom/31 天 overlap、durable checkpoint、Correction、Agent Context Package v2、Gateway/Hermes/Obsidian corrected view，并补齐统一 Continuity Delta、未受影响 Story/Thread/Chapter 身份稳定、并发安全 dirty generation、Primary-only Current Project State 和 Context revision 联动。冻结 30/30 execution map、T0–T7、Backend/H2 668、PostgreSQL 16、Frontend/Playwright、Hermes、Obsidian、敏感内容、普通同头 CI、merge 后 master CI 与干净主线启动器均通过。Luna、DeepSeek、Qwen 各自在同一功能 Head 完成独立盲评 12/12、Chapter 9/9 和 continuity 3/3；旧失败保留。Owner-approved 结论为 `PASS_BY_OWNER_APPROVED_AUTOMATED_AND_INDEPENDENT_SEMANTIC_REVIEW`，原 12 场景 worksheet 仍为 `NOT_REVIEWED`，没有伪造分数或 HUMAN_REVIEW_PASS。V3.10 只在 backfill 合并、最终 master 验证与 V3.9 清理后进入；继续禁止 Tag 和 Release。

V3.8.5 Final Chapter Closure 当前状态：Representative Cluster、dominant/co-dominant/minor、代表覆盖、Claim ceiling、有界 repair 与保守 split 工程链已完成。源码头 `e1b67f28428e73f39fc23aa6f85961155a20ffd8` 的同头 run `32609107531` 中，GPT 5.6 Luna Responses/max、DeepSeek V4 Flash Chat/max、Qwen3.7 Plus Messages/max 均通过 qualification 19/19 和 Chapter scenarios 9/9；没有替代 Qwen 模型。Final Chapter 已冻结为三 Provider 各 4 个、共 12 个 Chapter；Round 3 的 30 Story/8 Chapter 与 canonical-LF 哈希不变，展示变更 30/30 的 Truth/Evidence semantic hash 不变。2026-08-24 项目所有者明确批准最终 package 与 merge，并明确豁免本轮量化人工评分；独立签字结论为 `PASS_BY_EXPLICIT_OWNER_OVERRIDE`。PR #15 已以 `29c154eb618ca43edf58c631c14cc1d296e14f3f` 合入 master，merge 后 run `32652683003` 和干净 master 根启动器均通过；acceptance-backfill PR #16 已合并为最终 master `ab29b1ff0f842c029b5cf121bd584bd40fcf74b2`，两条 V3.8.5 分支与临时 worktree 已清理。冻结工件继续保持人工字段空白和 reviewerCount=0，独立签字 reviewerCount=1，不得伪造数值分数；仍禁止 Tag 和 Release。
...
ProjectFlow V3.9 is a local-first project-continuity, strong-fact and shared project-memory system for anyone who uses a computer to conduct a project. It turns real project materials into a human-readable history and then incrementally maintains that understanding as the project changes, while preserving ProjectFact as the only strong-fact source. Replaceable understanding, history snapshots and Current Project State remain separate from durable Project Facts, compatibility Timeline, optional Capability Map, Agent candidates, Project Memory Gateway, Hermes reads and Obsidian projection.
