...
V3.10 Release Readiness 已在 PR #19 完成 Flyway release schema ownership、精确 legacy schema fail-closed、H2 升级前备份/隔离恢复、PostgreSQL 备份确认、OS-backed Provider credential、loopback/external 运行安全、稳定用户数据目录、source-independent Windows portable runtime 与供应链/Windows gate。2026-08-29 收口轮次又由 exact V3.9 final `dd5ee41b6afcbd7703fa0883dc115c11f4821447` 应用实际生成 H2 和 PostgreSQL 16 旧库，证明二者都精确匹配 `KNOWN_V39`，并通过备份/恢复、controlled V1 baseline、V2、凭据迁移、保护记录不变和重启幂等性。这项证明不使用 V1 SQL 生成旧库，也没有放宽 schema signature。实现证据 Head `3c3dac91160b23f52e4ea680e423e3886dbda8ed` 的 Linux required CI、Windows portable 与 Luna/Responses、DeepSeek/Chat、Qwen/Messages secure credential-path smoke 均已通过。在最终文档 Head CI、PR merge、master 验证和 acceptance backfill 全部完成前，V3.10 仍不宣称 FINAL。继续 NO TAG、NO RELEASE，本轮不实现 V4 GUI。
