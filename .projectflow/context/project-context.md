# ProjectFlow V3.4.1 context

ProjectFlow 的核心定位是“自动维护项目从创建至今的长期记忆”。主流程为“分析新变化 → DevelopmentSegment → 自动 ProjectFact → 项目记录 / 项目记忆 → 自动项目历程”。正常证据充分事实自动记录，只有证据冲突、严重不足、事实边界不完整或无法安全去重等异常进入 NEEDS_ATTENTION。
...
ProjectChange、ProjectSediment、ProjectReviewCursor、旧 ProjectMemory 和 DevLog 仅作兼容，不再是新扫描或主时间视图。全生命周期能力地图、Hermes 和 Obsidian 正式同步属于后续阶段。
...
V3.4.1 不重新设计 V3.3.7 job infrastructure 或 V3.3.8 model gateway。真实 Provider、固定模型自动化和人工质量抽样必须分开记录。任何诊断都不得保存 Key、Authorization、reasoning 原文、完整 prompt、原始响应或未脱敏源码。
