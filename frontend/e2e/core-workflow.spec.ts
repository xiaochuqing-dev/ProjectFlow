...
type Job = { id: string; jobType: string; status: string; requestCount: number; inputSummary?: string; retriedFromJobId?: string | null };
...
test("自动事实不会伪装成待确认沉淀，能力分析保持显式前置条件", async ({ page, request }) => {
  ...
  const facts = await api<PageResult<Fact>>(request, "GET", `/projects/${fixture.projectId}/facts?page=0&size=20`);
...
test("任务取消与 retry 复用等价活动分析任务", async ({ request }) => {
  ...
  await expect.poll(async () => {
    ...
    failedJobId = jobs.find((job) => job.jobType === "PROJECT_TIMELINE_REFRESH" && job.status === "FAILED")?.id || "";
    return failedJobId;
  }, { timeout: 30_000 }).not.toBe("");

  await request.post(`${modelControl}/reset`);
  await request.post(`${modelControl}/delay-next`, { data: { count: 1, ms: 2500, task: "timeline" } });
  const active = await api<Job>(request, "POST", `/analysis-jobs/${failedJobId}/retry`);
  const retried = await api<Job>(request, "POST", `/analysis-jobs/${failedJobId}/retry`);
  ...
  expect(jobs.map((job) => job.id)).toContain(active.id);
