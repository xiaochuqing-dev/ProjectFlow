...
export default function ArchiveChangesPage() {
  return (
    <Suspense fallback={<AppShell eyebrow="项目资产更新记录" title="项目资产更新"><div className="min-h-[calc(100vh-4rem)] bg-surface p-6"><div className="h-1 bg-slate-950" /></div></AppShell>}>
...
function ArchiveChangesPageContent() {
  ...
  useEffect(() => {
    ...
    listProjectEvolutionRecords(session.accessToken, selectedProjectId)
      .then(setRecords)
      .catch((exception) => setError(exception instanceof Error ? exception.message : "项目资产更新加载失败"))
      .finally(() => setLoading(false));
  }, [selectedProjectId]);

  return (
    <AppShell eyebrow="项目资产更新记录" title={project ? `${project.name} · 项目资产更新` : "项目资产更新"}>
      <div className="min-h-[calc(100vh-4rem)] bg-surface p-6">
        <section className="mb-5 flex flex-wrap items-center justify-between gap-3 rounded-md border border-line bg-white p-4 shadow-panel">
          <div>
            ...
            <h2 className="text-xl font-semibold text-slate-950">项目资产更新</h2>
            <p className="mt-1 text-sm text-muted">展示项目资产每次更新改了哪些内容。</p>
          </div>
        ...
        <ResourceTimeline emptyText="暂无项目资产更新。采纳结构化变更或保存项目资产后会出现记录。" items={archiveItems} title={`变化记录 · ${records.length} 条`} />
      </div>
    </AppShell>
  );
}
...
function toArchiveChangeItem(record: ProjectEvolutionRecord): ResourceTimelineItem {
  ...
  return {
    id: record.id,
    title: record.summary || "项目资产更新",
    summary: record.detectedChanges || record.keyAchievements || "本次更新已沉淀到项目资产。",
    date: record.createdAt,
    type: record.technicalDecisions ? "技术决策" : record.keyIssues ? "风险变化" : "项目资产更新",
    status: "已入档",
