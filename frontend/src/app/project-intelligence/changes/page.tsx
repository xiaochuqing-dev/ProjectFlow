...
export default function ArchiveChangesPage() {
  return (
    <Suspense fallback={<AppShell eyebrow="项目档案变更记录" title="档案变化"><div className="min-h-[calc(100vh-4rem)] bg-surface p-6"><div className="h-1 bg-slate-950" /></div></AppShell>}>
...
function ArchiveChangesPageContent() {
  ...
  useEffect(() => {
    ...
    listProjectEvolutionRecords(session.accessToken, selectedProjectId)
      .then(setRecords)
      .catch((exception) => setError(exception instanceof Error ? exception.message : "档案变化加载失败"))
      .finally(() => setLoading(false));
  }, [selectedProjectId]);

  return (
    <AppShell eyebrow="项目档案变更记录" title={project ? `${project.name} · 档案变化` : "档案变化"}>
      <div className="min-h-[calc(100vh-4rem)] bg-surface p-6">
        <section className="mb-5 flex flex-wrap items-center justify-between gap-3 rounded-md border border-line bg-white p-4 shadow-panel">
          <div>
            ...
            <h2 className="text-xl font-semibold text-slate-950">档案变化</h2>
            <p className="mt-1 text-sm text-muted">展示项目档案每次更新改了哪些内容。</p>
          </div>
        ...
        <ResourceTimeline emptyText="暂无档案变化。采纳结构化变更或保存项目档案后会出现记录。" items={archiveItems} title={`变化记录 · ${records.length} 条`} />
      </div>
    </AppShell>
  );
}
...
function toArchiveChangeItem(record: ProjectEvolutionRecord): ResourceTimelineItem {
  ...
  return {
    id: record.id,
    title: record.summary || "档案更新",
    summary: record.detectedChanges || record.keyAchievements || "本次更新已沉淀到项目档案。",
    date: record.createdAt,
    type: record.technicalDecisions ? "技术决策" : record.keyIssues ? "风险变化" : "档案更新",
    status: "已入档",
