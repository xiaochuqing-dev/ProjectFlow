...
function DailySourcesPageContent() {
  ...
  const sourceItems = useMemo<ResourceTimelineItem[]>(() => buildDailyReviewSourceItems({
    completedTasks,
    date,
    dayEvolution,
    dayLogs,
    ...
    ...item,
    href: `/dev-logs/sources/${item.id}?projectId=${selectedProjectId}&date=${date}`,
  })), [completedTasks, date, dayEvolution, dayLogs, memory, selectedProjectId]);
  ...
  return (
    <AppShell eyebrow="每日回顾来源明细" title={project ? `${project.name} · ${date}` : "每日回顾来源"}>
      <div className="min-h-[calc(100vh-4rem)] bg-surface p-6">
        ...
        <ResourceTimeline
          emptyText="当天还没有可用于每日回顾的来源。保存开发日志或确认项目沉淀后会出现在这里。"
          items={sourceItems}
